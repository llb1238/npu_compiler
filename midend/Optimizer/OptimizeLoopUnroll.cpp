#include "../../midend/Optimizer/OptimizeLoopUnroll.h"
...
void natureloop::determineLoopType()
{
    ...
    if (loopIncreaseValue.count(lhs) == 1) {
        if (lhs->value.tag == RVT_PHI && lhs->value.phi.phi[0].second->value.tag == RVT_INTEGER &&
...
void natureloop::cal_loop(RawBasicBlock * start, RawBasicBlock * end)
{
    ...
    while (!stack_bbs.empty()) {
        RawBasicBlockP pbb = stack_bbs.top();
        stack_bbs.pop();
