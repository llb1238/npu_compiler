...
void ClearInst(RawFunction *& function)
{
    ...
    bbs.erase(remove_if(bbs.begin(), bbs.end(), [](RawBasicBlock * bb) { return bb->isDeleted == true; }), bbs.end());
    for (auto bb: bbs) {
        auto & insts = bb->inst;
        auto & phis = bb->phi;
        insts.erase(remove_if(insts.begin(), insts.end(), [](RawValue * data) { return data->isDeleted == true; }),
                    insts.end());
        phis.erase(remove_if(phis.begin(), phis.end(), [](RawValue * data) { return data->isDeleted == true; }),
                   phis.end());
    }
}
...
void DCE(RawFunction *& function)
{ // 目前来看这里没有办法直接删除，还是得遍历一遍后删除
    if (function->basicblock.size() == 0)
        return;
    vector<RawValue *> W;
