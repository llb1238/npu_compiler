...
void Visit_Branch(const RawValueP & value)
{
    ...
    cout << "  br " << Symbol_List[branch.cond] << ", %" << branch.true_bb->name << ", %" << branch.false_bb->name
         << endl;
}
...
void Name_aggregate(const RawValueP & value)
{ // 这里可能要考虑一下
    string content = "{";
    ...
    for (auto elem: elements) {
        ...
        if (tag == RVT_INTEGER) {
            content += to_string(elem->value.integer.value);
        } else if (tag == RVT_FLOAT) {
            content += to_string(elem->value.floatNumber.value);
        } else {
            Name_Value(elem);
            content += Symbol_List[elem];
        }
